...
class Docolatte {
	...
	constructor() {
		...
		this.actions = {
			...
			OUTDIR_READY: outdir => {
				// populating import files from node_modules
				let min = this.config.minify ? '.min' : '';
				let imports = [
					{ type: 'module', src: 'feather-icons/dist/feather-sprite.svg', dst: 'assets/feather-sprite.svg' },
					{ type: 'module', src: `simplebar/dist/simplebar${min}.css`,    dst: `styles/simplebar${min}.css` }
				];
				if (this.config.code.theme) {
					imports.push({
						type: 'module',
						src:  `highlight.js/styles/${this.config.code.theme}.css`,
						dst:  `styles/hljs/${this.config.code.theme}.css`
					});
				}
				// import modules and user specified files
				console.log(`Importing files...`)
				for (let item of imports.concat(this.config.imports)) {
					let { type = 'asset', src, dst } = item;
					if (!src) throw error(`invalid entry of import; 'src' property is missing`, 'ConfigError');
					if (!dst) dst = path.join(type + 's', path.basename(src));
					let _src = (type == 'module') ? require.resolve(src) : path.join(this.env.pwd, src);
					let _dst = path.join(outdir, dst);
					...
					mkPath(path.dirname(_dst));
					copyFileSync(_src, _dst);

					if (type in this.files) this.files[type].push(dst);
				}
			},
