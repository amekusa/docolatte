...
const $ = require('./TmplUtil').instance();
...
class Docolatte {
	...
	buildNav(members) {
		...
		// build nav menus
		for (let k in this.config.nav.menus) {
			let nav = {
				refs, data: members[k],
				heading: this.config.nav.menus[k],
				wrap: [`<nav>`, `</nav>`]
			};
			if (k == 'tutorials') {
				...
				let link = helper.createLink('global');
				if (link) nav.heading = `<a href="${link}">` + nav.heading + `</a>`;
