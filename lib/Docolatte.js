...
class Docolatte {
			...
			INIT_DATA: data => {
				...
				if (this.config.data.exclude.length) {
					...
					if (excludes.count()) {
						...
						console.table(excludes.get(), ['___id', 'kind', 'longname', 'memberof']);
						excludes.remove();
					}
				}
				...
				if (this.config.data.removeOrphans) {
					...
					if (orphans.count()) {
						...
						console.table(orphans.get(), ['___id', 'kind', 'longname', 'memberof']);
						orphans.remove();
					}
				}
			},
	...
	buildNav(members) {
		let r = '';
		r += this.tmpl.partial('search-box.tmpl');

		// build nav menus
		let refs = []; // refs to menu items
		[
			{ data: members.tutorials,  heading: 'Tutorials', getLink: doclet => ({
				url: helper.tutorialToUrl(doclet.name)
			})},
			{ data: members.modules,    heading: 'Modules' },
			{ data: members.externals,  heading: 'Externals' },
			{ data: members.namespaces, heading: 'Namespaces', getLink: doclet => ({
				label: doclet.longname.replace(/^module:/, '')
			})},
			{ data: members.classes,    heading: 'Classes' },
			{ data: members.interfaces, heading: 'Interfaces' },
			{ data: members.events,     heading: 'Events' },
			{ data: members.mixins,     heading: 'Mixins' },

		].forEach(I => {
			let nav = new Nav(I.data, I.heading, {
				db:      this.data,
				refs:    refs,
				getLink: I.getLink
			});
		...
		this.searchDB = new SearchDB({
			...
			search: {
				keys: [
					...
					{ name: 'meta.shortpath', weight: 6 },
					{ name: 'classdesc',      weight: 4 },
