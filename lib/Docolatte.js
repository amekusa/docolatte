...
const { error, arr, merge, toLiteral } = require('./util');
		...
		this.filters = {
			...
			FINAL_HTML: r => {
				...
				r = r.replace('<!-- %DOCOLATTE::DATA_GOES_HERE% -->', this.$.elems(
					['script', { id: 'docolatte-search-items',   type: 'application/json' }, this.searchDB.items],
					['script', { id: 'docolatte-search-index',   type: 'application/json' }, this.searchDB.index],
					['script', { id: 'docolatte-search-options', type: 'application/json' }, this.searchDB.options]
				));
				return r;
			}
		};
